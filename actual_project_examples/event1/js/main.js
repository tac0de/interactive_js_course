...
const allElements = document.querySelectorAll(".contents > section:not(.sticky)");
let setHeights = [];

const getHeights = () => {
  setHeights = Array.from(allElements).reverse().reduce(
    (acc, curr) => [...acc, curr.offsetTop + curr.clientHeight],
    []
  );
};
...
const SHARE_INFO = {
  ...
  kakaoImg: 'https://img.joongang.co.kr/pubimg/event/2025/plusReport/sns_kakao@2x.min.png',
  imgWidth: 610,
  imgHeight: 319,
};

const UTM = {
  kakao: '?utm_source=kakao&utm_medium=share&utm_campaign=plus_report_2024&utm_term=&utm_content=report_2501',
  facebook: '?utm_source=facebook&utm_medium=share&utm_campaign=plus_report_2024&utm_term=&utm_content=report_2501',
  twitter: '?utm_source=twitter&utm_medium=share&utm_campaign=plus_report_2024&utm_term=&utm_content=report_2501',
...
const nativeShare = (platform) => {
  ...
  location.href = `joongangilbo://article/share?url=${encodeURIComponent(url)}&title=${encodeURIComponent(SHARE_INFO.text)}&img=${encodeURIComponent(SHARE_INFO.img)}`;
};
...
const share = (platform) => {
  ...
  if (platform === 'facebook') {
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, 'Share_Facebook', 'width=550,height=500');
  }

  if (platform === 'twitter') {
    window.open(`https://twitter.com/intent/tweet?text=${encodeURI(SHARE_INFO.text)}&url=${encodeURIComponent(url)}`, 'twitter', 'width=550,height=500');
  }
...
const setTabs = () => {
  ...
  const onChangeTab = (e) => {
    ...
    const panel = grand.querySelector(`#${target.getAttribute('aria-controls')}`);
    if (panel) {
      panel.hidden = false;
      panel.tabIndex = 0;
  ...
  const kbdNavigation = (e) => {
    ...
    tabFocus = key === KEY.RIGHT ? (tabFocus + 1) % tabs.length : (tabFocus - 1 + tabs.length) % tabs.length;
